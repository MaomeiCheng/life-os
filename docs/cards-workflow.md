...
## Notes
- Thumbnail quality upgrades (v3: avoid black/blur/transitions; sharpness/edge score; optional OCR/face/text)
  live in `docs/next-steps.md`.
- Hover preview is muted by default; click/tap the sound icon to toggle sound (browser gesture rules apply).
- If you edit crown item titles (MusicCrownItem), rerun step 4 to refresh card titles.
- iOS in-app browser may reuse the same tab/window for `_blank`.
  We use a unique window name to avoid "back to previous card" behavior.
