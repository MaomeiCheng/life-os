...
### TODO（命名/資料語意）
- Music items/pending 的「備註」目前 DB/API 欄位名仍為 `reason`（UI 顯示為備註）。
  正式資料庫/對外 schema 時建議 rename 為 `note`/`remark`（並規劃 migration + API 路徑/欄位相容）。
...
6. apps/web/README.md（若不存在就用下列檔案代替）
...
- 不要在 `bash -lc '...'` 裡面塞多層引號或 heredoc，容易被 shell 提前結束。
- 建議做法：先把腳本/程式寫入檔案（cat > file <<'EOF' ... EOF），再執行。
- 一律使用 `set -euo pipefail`，並在需要的地方加上 `|| true` 或明確錯誤處理。

## 2026-02-08 Security hardening (env + R2 + docs)
- Env files are now ignored and untracked:
  - apps/web/.env
  - apps/web/.env.local
  - apps/web/scripts/cards/.env.local
...
- Safe shell pattern noted:
  - Prefer writing to files (or single-quoted heredocs) and then executing.
...
- Untracked any previously-tracked env files (git rm --cached)
  and verified ignore rules apply.

- Redacted Cloudflare R2 public base / endpoint / bucket name from:
  - README.md
  - scripts/update-readme-r2.sh
  (placeholders kept)
...
- Status: security hardening + placeholder scans completed.
- No non-placeholder CF_R2_* / R2_BUCKET found in tracked files.
- Note: If past midnight, follow nightly wrap-up checklist.

## 2026-02-08 (Late UI + DB)
- Fixed UI: icon-only open button for cards.
- Dev server initially hit Prisma ECONNREFUSED (DB not reachable).
  Resolved by starting `lifeos-postgres`.
...
- [x] Security hardening (2026-02-08): env ignored + R2 redactions + DB creds redacted
